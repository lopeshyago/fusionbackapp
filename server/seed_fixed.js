...
async function seedDatabase() {
  try {
    ...
    const adminResult = await new Promise((resolve, reject) => {
      db.run(`
        INSERT OR IGNORE INTO users (email, password_hash, full_name, plan_status)
        VALUES (?, ?, ?, ?)
      `, ['admin@fusion.com', hashedPassword, 'Administrador Fusion', 'active'], function(err) {
        if (err) reject(err);
        else resolve(this);
      });
    });
    ...
    const instructorResult = await new Promise((resolve, reject) => {
      db.run(`
        INSERT OR IGNORE INTO users (email, password_hash, full_name, plan_status)
        VALUES (?, ?, ?, ?)
      `, ['instrutor@fusion.com', hashedPassword, 'Instrutor Fusion', 'active'], function(err) {
        if (err) reject(err);
        else resolve(this);
      });
    });
    ...
    const studentResult = await new Promise((resolve, reject) => {
      db.run(`
        INSERT OR IGNORE INTO users (email, password_hash, full_name, plan_status)
        VALUES (?, ?, ?, ?)
      `, ['aluno@fusion.com', hashedPassword, 'Aluno Fusion', 'active'], function(err) {
        if (err) reject(err);
          else resolve(this);
      });
    });
